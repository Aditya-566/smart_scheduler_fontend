...
export default function Rooms() {
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            setRooms([
                { id: 1, number: '101', capacity: 60, type: 'LECTURE', features: ['Projector', 'Whiteboard'] },
                { id: 2, number: '102', capacity: 30, type: 'LAB', features: ['Computers', 'Smartboard'] },
                { id: 3, number: '201', capacity: 120, type: 'SEMINAR', features: ['PA System', 'Stage'] }
            ]);
            setIsLoading(false);
        }, 1000);
    }, []);
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                {isLoading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6 bg-slate-50/50">
                        {rooms.map(room => (
                            <div key={room.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-emerald-300 transition-all group">
